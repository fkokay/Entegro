...
namespace Entegro.Engine
{
    ...
    {
        const string EntegroNamespace = "Entegro";
        ...
        class EngineStarter : EngineStarter<SmartEngine>
        {
            ...
            protected override IEnumerable<Assembly> ResolveCoreAssemblies()
            {
                ...
                var appAssemblies = AssemblyLoadContext.Default.Assemblies
                    .Where(x => x.FullName.StartsWith(EntegroNamespace) && IsCoreDependency(x.GetName().Name))
                    .Select(x => new
                    {
                        Name = x.GetName(),
                        Assembly = x
                ...
                {
                    return (name == EntegroNamespace || name.StartsWith(nsPrefix)) &&
