...
using Entegro.Diagnostics;
using Entegro.Engine;
using Entegro.Engine.Runtimes;
using Entegro.Http;
using Entegro.Threading;
using Entegro.Utilities;

namespace Entegro.Pdf.WkHtml
{
    ...
    public class WkHtmlToPdfConverter : IPdfConverter
    {
        ...
        private static async Task<string> GetToolNameAsync()
        {
            ...
            try
            {
                ...
                if (!fi.Exists)
                {
                    ...
                    fi = await libraryInstaller.InstallFromPackageAsync(new InstallNativePackageRequest("wkhtmltopdf", true, "Entegro.wkhtmltopdf.Native"));
                }
        ...
        private async Task RunProcessAsync(string arguments, IPdfInput input, CancellationToken cancelToken)
        {
            ...
            DataReceivedEventHandler onDataReceived = (o, e) =>
            {
                ...
                if (e.Data.HasValue())
                {
                    if (!e.Data.Contains("libpng warning"))
                    {
                        data.Add(e.Data);
