...
(function () {
    ...
    dzOpts.dictDefaultMessage = "Yüklemek için dosyaları buraya bırakın";
    dzOpts.dictFallbackMessage = "Tarayıcınız drag'n'drop dosya yüklemelerini desteklemiyor.";
    dzOpts.dictFallbackText = "Tarayıcınız drag'n'drop dosya yüklemelerini desteklemiyor.";
    dzOpts.dictFileTooBig = "Dosya çok büyük ({{filesize}} MiB). Maks. Dosya boyutu: {{maxFilesize}} MiB.";
    dzOpts.dictInvalidFileType = "Bu türdeki dosyaları yükleyemezsiniz.";
    dzOpts.dictResponseError = "Sunucu {{statusCode}} koduyla yanıt verdi.";
    dzOpts.dictCancelUpload = "Yüklemeyi iptal et";
    dzOpts.dictUploadCanceled = "Yükleme iptal edildi.";
    dzOpts.dictCancelUploadConfirmation = "Bu yüklemeyi iptal etmek istediğinizden emin misiniz?";
    dzOpts.dictRemoveFile = "Dosyayı kaldır";
    dzOpts.dictMaxFilesExceeded = "Başka dosya yükleyemezsiniz.";
})();

(function ($) {
    var assignableFiles = [];
    var assignableFileIds = "";
    var dialog = Entegro.Admin ? Entegro.Admin.Media.fileConflictResolutionDialog : null;;
        ...
        return this.each(function () {
            ...
            if (opts.maxFiles === 1) {
                ...
                if (!currentFileId || currentFileId === 0) {
                    ...
                    // Load thumbnail.
                    Entegro.media.lazyLoadThumbnails(fuContainer.find('.fu-thumb'));
            ...
            el.on("addedfiles", function (files) {
                ...
                if (elStatusWindow.length > 0) {
                    ...
                    elStatusWindow.find(".current-file-text").text((files.length === 1 ? "dosya yükleniyor" : "dosyalar yükleniyor"));
            ...
            function setPreviewIcon(file, small) {
                ...
                var icon = Entegro.media.getIconHint(file);
            ...
            elRemove.on('click', function (e) {
                ...
                fuContainer.find('.fu-message').html("Dosyaları yüklemek için buraya bırakın veya tıklayın");
            ...
            function cancelAllUploads(removeFiles) {
                ...
                if (elStatusWindow.length > 0) {
                    ...
                    elStatusWindow.find(".current-file-text").text((currentlyUploading.length === 1 ? "yükleme iptal edildi" : "yüklemeler iptal edildi"));
    ...
    window.displaySingleFilePreview = function (file, fuContainer, options) {
        var preview = Entegro.media.getPreview(file, { iconCssClasses: "fa-4x" });
    ...
    function setSingleFilePreviewIcon(fuContainer, typeFilter) {
        ...
        for (var type of types) {
            ...
            icon = Entegro.media.getIconHint(o);
            if (!icon.isFallback) {
                break;
            }
        }

        icon = icon || Entegro.media.getIconHint({});
