...
namespace Entegro.Infrastructure.Catalog.Categories.Extensions
{
    public static class ITreeNodeQueryExtensions
    {
        ...
        public static IQueryable<ITreeNode> ApplyDescendantsFilter(this IQueryable<ITreeNode> query,ITreeNode node)
        {
            Guard.NotNull(node);
        ...
        public static IQueryable<ITreeNode> ApplyDescendantsFilter(this IQueryable<ITreeNode> query,int parentNodeId,bool deep = true,bool includeSelf = true)
        {
            if (parentNodeId <= 0)
            {
                return query;
        ...
        /// <param name="includeSelf"><c>true</c> = add the parent node to the result list, <c>false</c> = ignore the parent node.</param>
        public static IQueryable<ITreeNode> ApplyDescendantsFilter(this IQueryable<ITreeNode> query, string treePath, bool includeSelf = true)
        {
            Guard.NotNull(query);
